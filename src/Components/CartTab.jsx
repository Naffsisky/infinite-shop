...
function CartTab({ isCartOpen, closeCartSidebar }) {
  ...
  const fetchCartData = async () => {
    try {
      const response = await axios.get("http://ovh.prinafsika.world/cart/user");
  ...
  const handleDeleteItem = async (id, index) => {
    try {
      await axios.delete(`http://ovh.prinafsika.world/cart/${id}`);
      setCartItems((prevItems) => {
        const newItems = [...prevItems];
  ...
  const handleCheckout = async () => {
    try {
      const response = await axios.patch("http://ovh.prinafsika.world/updateCart", {
      ...
      <div className="flex flex-col h-full py-6 mx-auto w-full text-gray-800 bg-white rounded-3xl max-w-[480px]">
        <div className="flex gap-5 self-start ml-6 max-w-full text-lg font-semibold text-center text-violet-600 w-[273px]">
          <button onClick={closeCartSidebar}>
            <img loading="lazy" src="https://cdn.builder.io/api/v1/image/assets/TEMP/51533fbb960bb2f159a2a80a27f7030d02eecd65b5eea324d7d1581d91bba959?" className="shrink-0 aspect-square w-[27px] rounded-xl" alt="Close" />
          </button>
          ...
          {cartItems.map((item, index) => (
            <div key={item.id} className="flex gap-5 justify-between mt-6 w-full max-w-[431px]">
              <div className="flex gap-5 items-start text-sm">
                <img loading="lazy" srcSet={`${item.product.cardurl}`} className="shrink-0 w-20 aspect-square rounded-xl" alt="Product" />
                <div className="flex flex-col mt-1.5">
                  ...
                  <div className="flex gap-5 mt-3.5 text-center whitespace-nowrap">
                    <div className="flex gap-2 items-center">
                      ...
                      <div className="justify-center self-stretch px-3 py-1 rounded-xl border border-solid border-slate-500">{item.quantity}</div>
                    ...
                    <button onClick={() => handleDeleteItem(item.id, index)}>
                      <img loading="lazy" src="https://cdn.builder.io/api/v1/image/assets/TEMP/2aadf3dc98e7c57e364d48ab3008ccf73c4c8a1d71cc44df4460087dfd65a2ef?" className="shrink-0 my-auto w-6 aspect-square rounded-xl" alt="Remove" />
                    </button>
                  </div>
                </div>
              </div>
              <div className="self-end mt-16 text-base font-semibold text-right">Rp {parseInt(item.product.price) * item.quantity}</div>
            </div>
          ))}
        ...
        <div className="mt-6 px-6 w-full">
          ...
          <div className="flex w-full justify-center items-center mt-6">
            <div onClick={handleCheckout} className="px-6 py-3 w-full text-lg font-semibold text-center text-white bg-violet-600 rounded-xl max-w-[432px] cursor-pointer">
              Checkout
            </div>
          </div>
          <div

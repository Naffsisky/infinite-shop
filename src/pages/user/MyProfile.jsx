...
function MyProfile() {
  ...
  const handleImageChange = async (e) => {
    ...
    try {
      const response = await axios.patch("http://ovh.prinafsika.world/updateProfile", formData, {
        headers: {
  ...
  const handleSave = async () => {
    try {
      const response = await axios.patch("http://ovh.prinafsika.world/updateProfile", {
        name,
        email,
        phone,
        currentPassword,
  ...
  return (
    <div className="my-profile-container">
      ...
      <div className="Frame109 w-[921px] h-[635px] p-10 bg-white rounded-[20px] border border-violet-600 flex-col justify-start items-start gap-10 inline-flex mt-10">
        <div className="Frame107 justify-start items-center gap-9 inline-flex">
          <div className="Group219 w-[104px] h-[100px] relative">
            <div className="Ellipse10 w-[100px] h-[100px] left-0 top-0 absolute bg-purple-100 rounded-full overflow-hidden">
              {profileImage ? <img src={profileImage} alt="Profile" className="w-full h-full object-cover rounded-full" /> : <div className="w-full h-full bg-purple-100 rounded-full" />}
            </div>
            <input type="file" accept="image/*" className="hidden" id="profileImageInput" onChange={handleImageChange} />
            <label htmlFor="profileImageInput" className="cursor-pointer">
              <div className="Group204 w-7 h-7 left-[76px] top-[60px] absolute">
                ...
                <FontAwesomeIcon icon={faEdit} className="BasilEditOutline w-5 h-5 left-[4px] top-[4px] absolute" style={{ color: "white" }} />
              </div>
            </label>
          </div>
          <div className="text-gray-800 text-xl font-medium font-['Poppins']">{user && user.name}</div>
        </div>
        <div className="Frame106 flex-col justify-end items-end gap-8 flex">
          <div className="Frame101 justify-start items-center gap-[123px] inline-flex">
            <div className="Name text-gray-800 text-base font-normal font-['Poppins']">Name</div>
            <input
              type="text"
          ...
          <div className="Frame103 justify-start items-center gap-[116px] inline-flex">
            <div className="EMail text-gray-800 text-base font-normal font-['Poppins']">E-mail</div>
            <input
              type="email"
          ...
          <div className="Frame103 justify-start items-center gap-[50px] inline-flex">
            <div className="MobileNumber text-gray-800 text-base font-normal font-['Poppins']">Mobile Number</div>
            <input
              type="text"
          ...
          <div className="Frame103 justify-start items-center gap-[30px] inline-flex">
            <div className="Password text-gray-800 text-base font-normal font-['Poppins']">Current Password</div>
            <input
              type="password"
          ...
          <div className="Frame103 justify-start items-center gap-[60px] inline-flex">
            <div className="Password text-gray-800 text-base font-normal font-['Poppins']">New Password</div>
            <input
              type="password"
              className="password-text Frame91 w-[668px] h-11 px-5 py-3 rounded-[10px] border border-neutral-300 justify-start items-center gap-2.5 flex text-gray-800 text-base font-normal font-['Poppins']"
              value={newPassword}
      ...
      {showSuccessModal && <div className="backdrop fixed inset-0 bg-black bg-opacity-50 z-40"></div>}

      {showSuccessModal && (
        <dialog id="my_modal_8" className="modal modal-bottom sm:modal-middle border border-gray-300 shadow-lg z-50" open>
          <div className="w-[551px] h-[360px] relative bg-white rounded-2xl">
            <div className="left-[61.50px] top-[101px] absolute flex-col justify-start items-center gap-5 inline-flex">
              <div className="flex-col justify-start items-center gap-2 flex">
                ...
                <div className="w-44 text-center text-slate-500 text-sm font-normal font-['Poppins']">Your Account has Updated</div>
              </div>
              ...
              <form method="dialog" className="w-36 px-10 py-1 bg-green-600 rounded-full justify-center items-center gap-2.5 inline-flex">
                <a href="/MyProfile" className="text-center text-white text-base font-normal font-['Poppins']" onClick={() => setShowSuccessModal(false)}>
                  Continue
                </a>
              </form>
            </div>
            <img className="w-80 h-80 left-[211px] top-[10px] absolute" src="https://i.postimg.cc/HkcTC7BV/JEMPOL.png" alt="Success" />
          </div>
        </dialog>
      )}

      {showErrorModal && <div className="backdrop fixed inset-0 bg-black bg-opacity-50 z-40"></div>}

      {showErrorModal && (
        <dialog id="my_modal_4" className="modal modal-bottom sm:modal-middle border border-gray-300 shadow-lg z-50" open>
          <div className="w-[551px] h-[360px] relative bg-white rounded-2xl">
            <div className="left-[61.50px] top-[101px] absolute flex-col justify-start items-center gap-5 inline-flex">
              <div className="flex-col justify-start items-center gap-2 flex">
                <div className="text-center text-red-600 text-2xl font-bold font-['Poppins']">Error</div>
                <div className="w-44 text-center text-slate-500 text-sm font-normal font-['Poppins']">There was an error updating your profile. Please try again.</div>
                {errorData && <div className="w-44 text-center text-slate-500 text-sm font-normal font-['Poppins']">Error details: {errorData.message}</div>}
              </div>
              <div className="self-stretch h-px border border-neutral-500"></div>
              <button className="w-36 px-10 py-1 bg-red-600 rounded-full justify-center items-center gap-2.5 inline-flex text-white text-base font-normal font-['Poppins']" onClick={() => setShowErrorModal(false)}>
                Close
              </button>
            </div>
          </div>
